  ...
  CREATE OR REPLACE TEMP TABLE dataset AS (
    WITH
      ...
      ----
        SELECT
          ga.user_pseudo_id,
          ga.user_pseudo_id ||"/"||event_date||"/"|| (SELECT value.int_value FROM UNNEST(event_params) WHERE key = "ga_session_id") as session_id,
        ...
        WHERE
          SAFE.PARSE_DATE('%Y%m%d', _TABLE_SUFFIX) BETWEEN date_start - LOOKBACK_DAYS AND date_end + LOOKAHEAD_DAYS
        GROUP BY
          1, 2
      ),
